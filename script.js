...
// Step 3: Get the sensitive user information from the vault database
const vault = async (id) => {
    const sensitiveInfo = await new Promise((resolve) => {
        setTimeout(() => {
            // Return sensitive user information (credit card number, SSN, etc.)
            resolve({
                creditCardNumber: `************${id}`,
                ssn: `***-**-${id}`,
            });
...
async function getSensitiveInfo() {
    const id = 1; // Replace with the actual user ID
    const sensitiveInfo = await vault(id);
